// Package cloudflare implements a DNS provider CloudFlare
...
func InitProvider(email, apiKey string) (*DNSProvider, error) {
	return NewDNSProviderCredentials(email, apiKey)
}
...
func (c *DNSProvider) SetUp(domain, targetname, challenge string) error {
	...
	_, err = c.makeRequest("POST", fmt.Sprintf("/zones/%s/dns_records", utils.UnFqdn(zoneID)), bytes.NewReader(body))
	if err != nil {
		return err
	}

	err = c.checkLoop(domain, fqdn, challenge)
	if err != nil {
		return err
	}
...
func (c *DNSProvider) checkLoop(domain, fqdn, value string) error {
	timeout, interval := c.Timeout()
	logging.Log.Debugf("Checking DNS record propagation for %s...", domain)
	err := utils.WaitFor(timeout, interval, func() (bool, error) {
		return utils.CheckDNSPropagation(fqdn, value)
	})
	if err != nil {
		return err
	}
...
func (c *DNSProvider) CleanUp(domain, targetname, challenge string) error {
	fqdn := utils.DNS01Record(domain)
...
func (c *DNSProvider) getHostedZoneID(fqdn string) (string, error) {
	...
	result, err := c.makeRequest("GET", "/zones?name="+utils.UnFqdn(authZone), nil)
	if err != nil {
		return "", err
	}
...
func (c *DNSProvider) findTxtRecord(fqdn string) (*cloudFlareRecord, error) {
	...
	for _, rec := range records {
		if rec.Name == utils.UnFqdn(fqdn) && rec.Type == "TXT" {
			return &rec, nil
		}
	}
